 ...
 * Update URI: https://wordpress.org/
						...
						if ( $pack['value'] == 'yes' ) {
							if ( $prefix[0] == 'c' && is_numeric(substr($prefix, 1)) ) {
								if (!$hasEnqueuedCustom) {
									...
									if ($customIconsLastMod !== false) {
										wp_enqueue_style( 'ags-divi-icons-custom-icons', content_url('wadip-custom/icons.css'), null, self::PLUGIN_VERSION );
										wp_enqueue_script( 'ags-divi-icons-custom', content_url('wadip-custom/icons.js'), null, self::PLUGIN_VERSION );
									}
									$hasEnqueuedCustom = true;
								}
							} else if ( $prefix !== 'fa' ) {
