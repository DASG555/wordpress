...
/**
*   $prefix - used for option name
*   $name   - icon pack name
*   $enabled - input state - for premium packs should be set to false  in free version
*   $value   - option value
*/
function ags_wadip_return_label($prefix, $name, $enabled, $value = false, $sample=null, $allowDelete=false ) {
	if ($sample) {
		$sampleHtml = '<span class="agsdi-icon-set-sample">';
		foreach ($sample as $iconCode) {
			$sampleHtml .= '<span data-icon="'.esc_attr($iconCode).'"></span>';
		}
		$sampleHtml .= '</span>';
	}
	...
	$output = sprintf(
		'<label>
            <input type="checkbox" class="agsdi_icons_input" name="agsdi_%1$s_icons" value="%2$s" %3$s %4$s>
			%7$s
		...
		esc_attr($prefix),
		$enabled ? 'yes' : '',
		$enabled && $value === 'yes' ? 'checked' : '',
		$enabled ? '' : 'disabled',
		esc_html($name),
		$enabled ? '' : '<span class="ds_icon_expansion_pro">PRO</span>',
		$sample ? $sampleHtml : '',
		$allowDelete ? '<input type="hidden" name="_delete[]" value="'.esc_attr($prefix).'"><a href="#" class="agsdi_icons_delete">Delete</a>' : ''
	);
    ...
    <div id="ds_icon_expansion-settings">
        ...
        <div id="ds_icon_expansion-settings-tabs-content">
			...
			// Custom icon uploads and changes are processed in the main plugin file
                ...
                <form id="agsdi-icon-sets" method="post">
                    ...
                    <div class="agsdi-load-icon-sets-wrapper">
		                ...
		                foreach ( AGS_Divi_Icons::$icon_packs['multicolor'] as $key => $pack ) {
			                echo ags_wadip_return_label($key,  $pack['name'], false, false );
		                }
		                ?>
                    </div>
